package org.groupe2.iactic5.model;
...
import org.group2.iatic.ihm.models.*;
import org.group2.iatic.ihm.views.HomeView;
import org.groupe2.iactic5.model.views_actions.BookingsActionsImpl;
import org.groupe2.iactic5.model.views_actions.GuestsActionsImpl;
import org.groupe2.iactic5.model.views_actions.RoomsActionsImpl;
import org.groupe2.iactic5.model.views_actions.TimeSlotActionsImpl;

public class App extends Application {

    @Override
    public void start(Stage stage) throws IOException {
        PersonRepositoryInterface personRepository = new PersonFileRepository();
        RoomRepositoryInterface roomRepository = new RoomFileRepository();
        TimeSlotRepositoryInterface timeSlotRepository = new TimeSlotFileRepository();
        BookingRepositoryInterface bookingRepository = new BookingFileRepository(new PersonServices(personRepository), new RoomServices(roomRepository), new TimeSlotServices(timeSlotRepository));

        GuestsActions guestsActions = new GuestsActionsImpl(personRepository);
        RoomsActions roomsActions = new RoomsActionsImpl(roomRepository);
        TimeSlotActions timeSlotActions = new TimeSlotActionsImpl(timeSlotRepository);
        BookingActions bookingActions = new BookingsActionsImpl(bookingRepository, personRepository, roomRepository);

        Scene scene = new Scene(new HomeView(guestsActions, roomsActions, bookingActions, timeSlotActions));
